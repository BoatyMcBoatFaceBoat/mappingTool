// use join instead of merge to control full update pattern instead of merging the databound nodes with the appended nodes.
// add clippaths and tooltips to make text more readable 
...
function makeTree(){
  ...
  const t = d3.select('body').transition().duration(1000);
      ...
      let dataBoundNodes = tG.selectAll('.gNode')
        ...
        .join(
          enter => enter.append('g')
            ...
            .call(enter => enter.append('clipPath')
                                .attr('id', d => `clip${d.data.name}`)
                                .append('rect')
                                .attr('width', d => d.boxWidth - textPadding)
                                .attr('height', d => d.boxHeight)
                                // .attr('x', -0.5 * boxWidth)
                                .attr('y', d => -0.5 * d.boxHeight))
          ...
          // exit => exit.select('rect').attr('fill', 'red'),
          exit => exit.call(exit => exit.select('rect')
                                        .attr('fill', 'red'))
