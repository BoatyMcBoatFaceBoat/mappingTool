...
linkCntsPerLine.forEach(line => {
  if (!(/^#.*/).test(line)) { // all file path information
    ...
    if ((/^\d/).test(line)) { // file paths
      ...
      let bookName = pathSections[pathSections.length - 1]
      // bookLookup.push({
      //   // 'bookIndex': bookIndex,
      //   // 'name': pathSections[pathSections.length - 1]
      //   bookIndex: pathSections[pathSections.length - 1]
      // });
...
function Tree(treeData){
  ...
  this.createPrimaryRoot = function(sheets) {
    ...
    this.bookLookup = this.treeRoot.descendants().filter(node => {
      node.data.type == 'book' || node.data.type == 'sheet'
    });
    console.log(this.bookLookUp);
  ...
  this.createEdges = function(){
    // this.createBookEdges();
    // this.createSheetEdges();

  },

  this.createBookEdges = function(){
    bookLinks.forEach(bl => {
      this.bookEdges.push(bookRegister[bl[0]], )
    })
  }
